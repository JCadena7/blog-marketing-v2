...
const AdminSearchGlobal: React.FC<AdminSearchGlobalProps> = ({ isOpen, onClose }) => {
  ...
  // Stable flags to avoid using a function (hasPermission) in the effect deps
  const canSearchPosts = hasPermission('crear_post') || hasPermission('editar_post_cualquiera');
  const canSearchUsers = hasPermission('admin_completo');
  const canSearchComments = hasPermission('comentar') || hasPermission('admin_completo');
  const canSearchCategories = hasPermission('crear_categoria') || hasPermission('editar_categoria');
